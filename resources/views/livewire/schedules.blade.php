    ...
    <div>
        <div class="flex space-x-3 mb-3 items-center">
            ...
            <button type="button" wire:click="$set('selected', 'subject')" class="p-2 rounded text-white uppercase tracking-tight transition ease-in-out duration-500
                ...
                <button type="button" wire:click="$set('selected', 'custom')" class="p-2 rounded text-white uppercase tracking-tight transition ease-in-out duration-500
                {{ $selected == 'custom' ? 'bg-gray-500 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-300 transform hover:scale-105' }}">Custom</button>
        </div>
        @if($selected == "course")
