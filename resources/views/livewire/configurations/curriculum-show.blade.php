<div>
    <div class="w-full">
        ...
        <div class="flex justify-between mb-2">
            <a href="{{ route('courses.show', $curriculum->course) }}" class="bg-red-500 hover:bg-red-400 border px-2 py-1 text-sm text-white rounded-lg" >{{ __('Back') }}</a>
            <button class="bg-green-500 hover:bg-green-400 border px-2 py-1 text-sm text-white rounded-lg" wire:click="updateCurriculum">{{ __('Update') }}</button>
        </div>
        <div>
                                    ...
                                    <tbody class="divide-y">
                                        @foreach ($term as $k => $subject)
                                            <tr class="divide-x @error("subjects.$i.term.$j.$k.*") bg-red-200 @enderror">
                                                ...
                                                <td class="p-2">
                                                    <x-input class="w-full" wire:model="subjects.{{ $i }}.term.{{ $j }}.{{ $k }}.total" />
                                                </td>
                                                <td class="p-2">
